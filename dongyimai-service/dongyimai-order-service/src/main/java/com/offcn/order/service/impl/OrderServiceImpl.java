...
public class OrderServiceImpl extends ServiceImpl<OrderMapper, Order> implements OrderService {
    ...
    public void add(Order order) {
        ...
        for (Cart cart : cartList) {//得到每个商家对应的购物车数据
            long orderId = idWorker.nextId(); //为订单获取下一个id
        ...
        if ("1".equals(order.getPaymentType())){
            ...
            BigDecimal decimal = total_money1.multiply(BigDecimal.valueOf(100));
            payLog.setTotalFee(Long.valueOf(decimal.toString())); //数据表为big，实体类为long
    ...
    public void updateOrderStatus(String out_trade_no, String transaction_id) {
        ...
        for(String orderId:orderIds){
            ...
            if(order!=null){
                ...
                orderMapper.updateById(order);//修改后重新更新到数据库
            }
        }
        //清除redis缓存数据
